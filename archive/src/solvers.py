...
class ArgminSolver:
    ...
    def __init__(
        ...
        line_search_steps: tuple = (0.1, 0.05, 0.01, 0.005, 0.001),
    ):
    ...
    def _build_solver(self):
        ...
        Supports four modes:
        1. use_line_search=True: GD with multi-point line search (adaptive step size)
        2. use_adam=True: Riemannian Adam with per-coordinate adaptive LR
        3. momentum > 0: Riemannian Heavy Ball
        4. Otherwise: Vanilla Riemannian GD

        Accepts y_init for warm-starting (e.g., from logsumexp soft-argmin).
        """
        use_line_search = self.use_line_search
        use_adam = self.use_adam and not use_line_search
        use_momentum = (not use_adam) and (not use_line_search) and (self.momentum > 0)

        if use_line_search:
            # GD with multi-point line search
            t_options = jnp.array(self.line_search_steps)
            ...
            def solve_one(psi_params, x, y_init):
                ...
                def objective_at_y(y):
                    """Compute objective: 0.5 * d(x,y)^2 - psi(y)"""
                    d = self.manifold.dist(x, y)
                    psi_val = self._psi_scalar(psi_params, y)
                    return 0.5 * d**2 - psi_val
                    ...
                    def do_step(_):
                        ...
                        def try_step(t):
                            y_new = self.manifold.projx(
                                self.manifold.exponential_map(y, -t * g)
                            )
                            return objective_at_y(y_new)

                        obj_values = jax.vmap(try_step)(t_options)
                        best_idx = jnp.argmin(obj_values)
                        best_t = t_options[best_idx]

                        y_new = self.manifold.projx(
                            self.manifold.exponential_map(y, -best_t * g)
                        )
                ...
                final_res = jnp.linalg.norm(
                    self._stationarity_residual(psi_params, x, y)
                )
                return y, final_res
    ...
    def print_convergence_report(self, analysis: dict, plot: bool = False, **plot_kwargs):
        ...
        if self.use_line_search:
            opt_str = f"LineSearch({len(self.line_search_steps)} steps)"
        elif self.use_adam:
            opt_str = "Adam"
        elif self.momentum > 0:
            opt_str = f"HeavyBall(mom={self.momentum})"
        else:
            opt_str = "GD"
    ...
    def plot_residual_histogram(
        ...
        if title is None:
            if self.use_line_search:
                opt_str = f"LineSearch({len(self.line_search_steps)})"
            elif self.use_adam:
                opt_str = "Adam"
            elif self.momentum > 0:
                opt_str = f"HeavyBall(β={self.momentum})"
            else:
                opt_str = "GD"
