...
class ExpMapFlow(nn.Module):
    potential_cfg: Any  # dict or OmegaConf
    manifold: Manifold

    def setup(self):
        self.potential_mod = instantiate_from_cfg(
            self.potential_cfg, manifold=self.manifold,
        )
...
class SequentialFlow(nn.Module):
    n_transforms: int
    manifold: Manifold
    single_transform_cfg: Any  # dict or OmegaConf

    def setup(self):
        transforms = []
        for i in range(self.n_transforms):
            mod = instantiate_from_cfg(
...
def instantiate_from_cfg(cfg, **kwargs):
    """Simple replacement for hydra.utils.instantiate."""
    cfg_dict = dict(cfg)
    ...
    # Map target string to actual class
    if target == 'flows.InfAffine':
        return InfAffine(**cfg_dict)
    elif target == 'flows.RadialPotential':
        return RadialPotential(**cfg_dict)
    elif target == 'flows.MultiInfAffine':
        return MultiInfAffine(**cfg_dict)
    elif target == 'flows.ExpMapFlow':
        return ExpMapFlow(**cfg_dict)
    else:
