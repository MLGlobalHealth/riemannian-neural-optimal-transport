...
| Table 3 | Historical ablation driver with an explicit baseline; stored reference values and plotting | Stored values match the published numeric cells at printed precision. The full recovered ablation suite has not been freshly rerun. |
...
| Figure 4 | Recovered landmark-embedding computation and plotter | The original notebook kernel environment is unconfirmed; a fresh command and labeled historical values are provided. |
