...
def get_uniform(manifold):
    ...
    elif type(manifold).__name__ == "SO3":
        return SO3Uniform(manifold=manifold)
    elif type(manifold).__name__ == "SE3":
        from src.densities import SE3Uniform
        return SE3Uniform(manifold=manifold)
    else:
        assert False, f"get_uniform not implemented for {type(manifold)}"
...
class Earth(Density):
    ...
    def __post_init__(self):
        ...
        self.data = self.data.at[:, -1].set(-self.data[:, -1])
...
class EmpiricalSphereDensity(Density):
    """Empirical density on S2 from a CSV file with lat/lon columns."""
    csv_path: str = ""
    ...
    def __post_init__(self):
        ...
        self.weights = jnp.array(self.kde.weights)
