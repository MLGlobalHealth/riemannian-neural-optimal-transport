...
def render_embeddings(rows,plt):
    ...
    for (method,group),values in groups.items():
        ...
        label=method if str(group).startswith('historical') else method+' · '+str(group)[:6]
    ...
    axes[2].set_yscale('log');axes[2].axhline(1e-8,color='.5',ls=':',lw=1);axes[2].text(.02,.03,'Zeros displayed at 10⁻⁸; raw values retained',transform=axes[2].transAxes,fontsize=7)
    return fig
